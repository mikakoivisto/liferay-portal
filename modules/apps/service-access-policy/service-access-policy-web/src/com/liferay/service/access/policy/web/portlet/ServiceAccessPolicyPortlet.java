...
public class ServiceAccessPolicyPortlet extends MVCPortlet {

	public void deleteServiceAccessPolicy(
			ActionRequest actionRequest, ActionResponse actionResponse)
		throws Exception {

		long serviceAccessPolicyId = ParamUtil.getLong(
			actionRequest, "serviceAccessPolicyId");

		_serviceAccessPolicyService.deleteServiceAccessPolicy(
			serviceAccessPolicyId);
	}

	public void updateServiceAccessPolicy(
			ActionRequest actionRequest, ActionResponse actionResponse)
		throws Exception {
		...
		if (serviceAccessPolicyId > 0) {
			_serviceAccessPolicyService.updateServiceAccessPolicy(
				serviceAccessPolicyId, allowedServiceSignatures, name, titleMap,
				serviceContext);
		}
		else {
			_serviceAccessPolicyService.addServiceAccessPolicy(
